...
package body og is
   ...
   type ctxt_Ty is
      ...
      msg : aliased OctStr := (Data => (104, 105, others => 0), Length => 2);
   end record;
   ...
   procedure runTransition(Id: Integer) is
      ...
      tmp21 : aliased OctStr;
      tmp5 : aliased OctStr;
      tmp15 : aliased OctStr;
      tmp9 : aliased OctStr;
      tmp23 : OctStr;
      begin
         while (trId /= -1) loop
            case trId is
               ...
               when 2 =>
                  ...
                  tmp9.Data (1 .. 5 + 5) := ((104, 101, 108, 108, 111) & OctStr_Array'(119, 111, 114, 108, 100, others => <>)(1 .. 5));
                  tmp9.Length := 5 + 5;
                  RIÜsend_to_uart(tmp9'Access);
                  --  send_to_uart ('How' // 'are' // 'you?') (50,27)
                  --  !! stack: _call_external_function line 1290
                  tmp15.Data (1 .. 6 + 4) := ((((72, 111, 119) & OctStr_Array'(97, 114, 101, others => <>)(1 .. 3))) & OctStr_Array'(121, 111, 117, 63, others => <>)(1 .. 4));
                  tmp15.Length := 6 + 4;
                  RIÜsend_to_uart(tmp15'Access);
                  --  send_to_uart('I said' // msg // '!') (52,27)
                  --  !! stack: _call_external_function line 1290
                  tmp21.Data (1 .. 6 + ctxt.msg.Length + 1) := ((((73, 32, 115, 97, 105, 100) & ctxt.msg.Data(1..ctxt.msg.Length))) & OctStr_Array'(33, others => <>)(1 .. 1));
                  tmp21.Length := 6 + ctxt.msg.Length + 1;
                  RIÜsend_to_uart(tmp21'Access);
                  --  msg := if msg = 'hi' then 'ping' else 'Received: ' // msg // ' ' fi (54,25)
                  if OctStr_Equal(ctxt.msg, (Data => (104, 105, others => 0), Length => 2)) then
                     tmp23 := (Data => (112, 105, 110, 103, others => 0), Length => 4);
                  else
                     tmp23.Data(1..10 + ctxt.msg.Length + 1) := ((((82, 101, 99, 101, 105, 118, 101, 100, 58, 32) & ctxt.msg.Data(1..ctxt.msg.Length))) & OctStr_Array'(32, others => <>)(1 .. 1));
