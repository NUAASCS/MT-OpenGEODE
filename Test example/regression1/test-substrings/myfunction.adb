...
package body myfunction is
   ...
   type ctxt_Ty is
      ...
      seqbool2 : aliased asn1SccSeqBool2 := (Data => (1 => true, 2 => false, others => true), Length => 2);
      iseq : aliased asn1SccIntSeq := (Data => (1 => 0, 2 => 1, 3 => 2, 4 => 3, others => 0), Length => 4);
      seqof : aliased asn1SccMySeqOf := (Data => (1 => asn1Scchello, 2 => asn1Sccworld, others => asn1Scchello));
      seqint : aliased asn1SccSeqInt := (Data => (1 => 1, others => 1), Length => 1);
      seqbool : aliased asn1SccSeqBool := (Data => (1 => true, 2 => false, others => true));
      n : aliased asn1SccT_UInt8;
      fixedseq : aliased asn1SccFixedIntSeq := (Data => (1 => 0, 2 => 1, 3 => 2, 4 => 3, 5 => 4, others => 0));
         ...
         while (trId /= -1) loop
            case trId is
               when 0 =>
                  ...
                  Put(asn1SccT_UInt8'Image(ctxt.n));
                  New_Line;
                  ...
                  for elem in ctxt.seqof.Data'Range loop
                     if ctxt.seqof.Data(elem) = asn1Scchello then
                        tmp81 := True;
                     end if;
                  ...
                  for elem in ctxt.seqof.Data(1..1)'Range loop
                     if ctxt.seqof.Data(1..1)(elem) = asn1Scchello then
                        tmp91 := True;
                     end if;
                  ...
                  if (tmp91) = true then
                     ...
                     for elem in ctxt.seqof.Data(1..2)'Range loop
                        if ctxt.seqof.Data(1..2)(elem) = asn1Scchello then
                           tmp99 := True;
                        end if;
                  ...
                  begin
                     for x_idx in ctxt.seqof.Data'Range loop
                        ...
                        begin
                           --  writeln(num(x)) (105,9)
                           Put(Asn1UInt'Image(num_asn1SccMyEnum(x)));
                           New_Line;
                        end;
                     end loop;
                  end;
                  ...
                  begin
                     for x_idx in ctxt.seqof.Data(1..2)'Range loop
                        ...
                        begin
                           --  writeln(-num(x)) (113,9)
                           Put(AsN1INT'Image((-AsN1INT(num_asn1SccMyEnum(x)))));
                           New_Line;
                        end;
                     end loop;
                  end;
            ...
            ctxt.seqint.Data(1..ctxt.seqint.Length + 1) := ((ctxt.seqint.Data(1..ctxt.seqint.Length)) & asn1SccSeqInt_Array'(1 => 2, others => <>)(1 .. 1));
            ctxt.seqint.Length := ctxt.seqint.Length + 1;
            --  seqint := seqint(0,0) // {5} (182,13)
            ctxt.seqint.Data(1..1 + 1) := ((ctxt.seqint.Data(1..1)) & asn1SccSeqInt_Array'(1 => 5, others => <>)(1 .. 1));
