...
begin
    --set b 
    declare
        b_tmp:adaasn1rtl.Asn1Boolean;
    begin
        b_tmp := FALSE;
        val := Choice'(kind => b_PRESENT, b => b_tmp);
    end;
    return val;
end Choice_Init;
...
BEGIN
    case val.kind is
        WHEN b_PRESENT =>
            ret := adaasn1rtl.ASN1_RESULT'(Success => TRUE, ErrorCode => ERR_CHOICE_B);
        WHEN i_PRESENT =>
            ret := adaasn1rtl.ASN1_RESULT'(Success => TRUE, ErrorCode => ERR_CHOICE_I);
        WHEN f_PRESENT =>
            ret := adaasn1rtl.ASN1_RESULT'(Success => ((-1.00000000000000000000E+308 <= val.f) AND (val.f <= 1.00000000000000000000E+308)), ErrorCode => ERR_CHOICE_F);
    end case;
    RETURN ret;
...
IS
    ret : adaasn1rtl.ASN1_RESULT;
BEGIN
    ret := adaasn1rtl.ASN1_RESULT'(Success => (((((val = one)) OR ((val = two)))) OR ((val = three))), ErrorCode => ERR_ENUM_T);
    RETURN ret;
