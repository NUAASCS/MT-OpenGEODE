...
package body orchestrator is
   ...
   procedure pulse(pulse_param: access asn1SccT_Int) is
      begin
         case ctxt.state is
            when running =>
               runTransition(1);
   ...
   procedure arr(arr_param: access asn1SccT_SeqOf) is
      begin
         case ctxt.state is
            when running =>
               runTransition(1);
         ...
         while (trId /= -1) loop
            case trId is
               ...
               when 3 =>
                  ...
                  --  and counter = 0 (75,0)
                  --  ANSWER true (77,25)
                  if ((asn1SccT_SeqOf_Equal(ctxt.seqof, (Data => (1 => 4, 2 => 4, 3 => 4, 4 => 4, others => 4), Length => 4)) and (ctxt.counter = 0))) = true then
                     --  writeln( 'Property should be checked') (79,33)
                     Put("Property should be checked");
                     New_Line;
                     --  ANSWER false (81,25)
                  elsif ((asn1SccT_SeqOf_Equal(ctxt.seqof, (Data => (1 => 4, 2 => 4, 3 => 4, 4 => 4, others => 4), Length => 4)) and (ctxt.counter = 0))) = false then
                     --  seqof(1) := 1 (83,33)
                     ctxt.seqof.Data(2) := 1;
                  end if;
