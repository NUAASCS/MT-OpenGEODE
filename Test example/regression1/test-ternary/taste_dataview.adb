...
BEGIN
    ret := adaasn1rtl.ASN1_RESULT'(Success => TRUE, ErrorCode => ERR_BOOLTYPE);
    RETURN ret;
...
function MyStr_Equal(val1, val2: in MyStr)
    return Boolean 
is
begin
...
IS
    ret : adaasn1rtl.ASN1_RESULT;
BEGIN
    ret := adaasn1rtl.ASN1_RESULT'(Success => ((3 <= val.Length) AND (val.Length <= 1000)), ErrorCode => ERR_MYSTR);
    RETURN ret;
END MyStr_IsConstraintValid;
