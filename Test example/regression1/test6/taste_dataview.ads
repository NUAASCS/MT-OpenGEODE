...
TYPE MyChoice_selection IS (a_b_PRESENT, b_PRESENT);
for MyChoice_selection use
    (a_b_PRESENT => 1, b_PRESENT => 2);
for MyChoice_selection'Size use 32;

TYPE MyChoice(kind : MyChoice_selection:= a_b_PRESENT) IS 
RECORD
    case kind is
    when a_b_PRESENT =>
            a_b: adaasn1rtl.Asn1Boolean;
    when b_PRESENT =>
            b: MySeq;
    end case;
END RECORD;
...
ERR_MYCHOICE_A_B:CONSTANT INTEGER := 96; 
...
ERR_STRING:CONSTANT INTEGER := 183; -- 
FUNCTION String_IsConstraintValid(val : in String) return adaasn1rtl.ASN1_RESULT;
-- Autocompletion --------------------------------------------
-- Autocompletion_secondField --------------------------------------------

TYPE Autocompletion_secondField IS RECORD 
    third : adaasn1rtl.Asn1Boolean;
    fourth : adaasn1rtl.Asn1Boolean;
END RECORD;

TYPE Autocompletion IS RECORD 
    first_field : adaasn1rtl.Asn1Boolean;
    secondField : Autocompletion_secondField;
END RECORD;

function Autocompletion_secondField_Equal(val1, val2: in Autocompletion_secondField) return Boolean;

function Autocompletion_Equal(val1, val2: in Autocompletion) return Boolean;
...
ERR_AUTOCOMPLETION_FIRST_FIELD:CONSTANT INTEGER := 190; 
ERR_AUTOCOMPLETION_SECONDFIELD_THIRD:CONSTANT INTEGER := 197; 
ERR_AUTOCOMPLETION_SECONDFIELD_FOURTH:CONSTANT INTEGER := 204; 
...
mySeq : CONSTANT TASTE_Dataview.Autocompletion:= Autocompletion'(
    first_field => TRUE,
    secondField => Autocompletion_secondField'(
        third => TRUE,
        fourth => FALSE
...
myConst : CONSTANT adaasn1rtl.Asn1Int:= 5;
myStart : CONSTANT TASTE_BasicTypes.T_Int32:= 32; 
