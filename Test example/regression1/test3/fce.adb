...
package body fce is
   ...
   type ctxt_Ty is
      ...
      fce_stat_reg : aliased asn1SccFCE_RECOVERY_STATUS_REGISTER;
      fce_selected_counter : aliased asn1SccCounter_ty;
      new_rc : aliased asn1SccCounter_ty;
      fce_stop_control : aliased asn1SccBool_ty := false;
      fce_selected : aliased asn1SccBool_ty;
      fdir_enable : aliased asn1SccBool_ty;
      fce_ctrl_reg : aliased asn1SccFCE_CONTROL_FLAGS_STATUS_REGISTER;
      sgm_ram : aliased asn1SccFCE_SGM_RAM;
      arearraysdeployed : aliased asn1SccBool_ty;
      op_param : aliased asn1SccBool_ty;
      eeprom : aliased asn1SccFCE_SGM_EEPROM;
   end record;
   ctxt: aliased ctxt_Ty;
   CS_Only  : constant Integer := 9;
   procedure runTransition(Id: Integer);
   procedure p‹Bootup_actions(eeprom: in out asn1SccFCE_SGM_EEPROM);
   procedure p‹Bootup_actions(eeprom: in out asn1SccFCE_SGM_EEPROM) is
      tmp79 : aliased asn1SccFCE_SGM_EEPROM;
...
-- data in SKEL
         ...
         --  ANSWER ok (135,21)
         if (ctxt.fce_ctrl_reg.reference_pattern) = asn1Sccok then
            --  DECISION fce_ctrl_reg!arr_dep_flag (-1,-1)
            --  ANSWER majority (139,33)
            if (ctxt.fce_ctrl_reg.arr_dep_flag) = asn1Sccmajority then
            ...
            --  ANSWER nok (148,21)
         elsif (ctxt.fce_ctrl_reg.reference_pattern) = asn1Sccnok then
            --  COMMENT XXX
...
-- from the other RM register
   ...
   procedure Upload_Data(whatever: access asn1SccBool_ty) is
      begin
         case ctxt.state is
            when fce_test_mode =>
