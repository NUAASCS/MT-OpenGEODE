...
SUBTYPE Some_Thing is adaasn1rtl.Asn1Int range -100 .. 100;
...
TYPE SeqOf_array IS ARRAY (SeqOf_index) OF Some_Thing;
...
ERR_SEQOF_ELM:CONSTANT INTEGER := 71; 
ERR_SEQOF:CONSTANT INTEGER := 82; 
...
ERR_MYINTEGER:CONSTANT INTEGER := 57; -- 
...
ERR_MY_OCTSTR:CONSTANT INTEGER := 64; -- 
...
ERR_TOTO_ELEM_1:CONSTANT INTEGER := 103; 
ERR_TOTO_ELEM_2:CONSTANT INTEGER := 114; 
...
ERR_SEQBOOL_ELM:CONSTANT INTEGER := 131; 
ERR_SEQBOOL:CONSTANT INTEGER := 138; 
FUNCTION SeqBool_IsConstraintValid(val : in SeqBool) return adaasn1rtl.ASN1_RESULT;

default_seqof : CONSTANT TASTE_Dataview.SeqOf:= SeqOf'(Length => 3,Data => SeqOf_array'(1=>4, 2=>7, 3=>9, others => -100));
