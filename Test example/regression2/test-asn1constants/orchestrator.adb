...
package body orchestrator is
   ...
   type ctxt_Ty is
      ...
      tutu : aliased Enum_with_dash := first_one;
      titi : aliased NamedInt_with_dash := NamedInt_with_dash_second_value;
      toto : aliased NamedInt := NamedInt_one;
      recursive : aliased TASTE_Peek_id := myInt2;
   end record;
   ctxt: aliased ctxt_Ty;
   CS_Only  : constant Integer := 2;
   procedure runTransition(Id: Integer);
   procedure pulse(pulse_param: access NamedInt_with_dash) is
      begin
         case ctxt.state is
            when wait_for_gui =>
