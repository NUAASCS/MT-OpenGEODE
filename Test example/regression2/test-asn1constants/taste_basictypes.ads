...
SUBTYPE NamedInt is adaasn1rtl.Asn1UInt range 1 .. 2;


function NamedInt_Equal(val1, val2: in NamedInt) return Boolean;

function NamedInt_Init return NamedInt;

ERR_NAMEDINT:CONSTANT INTEGER := 181; -- 
FUNCTION NamedInt_IsConstraintValid(val : in NamedInt) return adaasn1rtl.ASN1_RESULT;
SUBTYPE Enum_with_dash_index_range is integer range 0..1;
TYPE Enum_with_dash IS (first_one, second_one) with Convention => C;
for Enum_with_dash use
    (first_one => 0, second_one => 1);

function Enum_with_dash_Equal(val1, val2: in Enum_with_dash) return Boolean;

function Enum_with_dash_Init return Enum_with_dash;

ERR_ENUM_WITH_DASH:CONSTANT INTEGER := 188; -- 
FUNCTION Enum_with_dash_IsConstraintValid(val : in Enum_with_dash) return adaasn1rtl.ASN1_RESULT;
SUBTYPE NamedInt_with_dash is adaasn1rtl.Asn1UInt range 0 .. 3;


function NamedInt_with_dash_Equal(val1, val2: in NamedInt_with_dash) return Boolean;

function NamedInt_with_dash_Init return NamedInt_with_dash;

ERR_NAMEDINT_WITH_DASH:CONSTANT INTEGER := 195; -- 
FUNCTION NamedInt_with_dash_IsConstraintValid(val : in NamedInt_with_dash) return adaasn1rtl.ASN1_RESULT;

myOne : CONSTANT TASTE_BasicTypes.NamedInt:= 1;
myBool : CONSTANT TASTE_BasicTypes.MyBoolean:= TRUE;
myInt : CONSTANT TASTE_BasicTypes.TASTE_Peek_id:= 5;
myInt2 : CONSTANT TASTE_BasicTypes.TASTE_Peek_id:= 5;
myString : CONSTANT TASTE_BasicTypes.VariableString:= VariableString'(Length => 4,Data => VariableString_array'(1=>16#DE#, 2=>16#AD#, 
                                                          3=>16#BE#, 4=>16#EF#, others=> 16#0#)) ;
mySeqBool : CONSTANT TASTE_BasicTypes.SeqBool:= SeqBool'(Length => 2,Data => SeqBool_array'(1=>TRUE, 2=>FALSE, others => FALSE));
myEnum : CONSTANT TASTE_BasicTypes.MyEnum:= eE;
myCh : CONSTANT TASTE_BasicTypes.MyChoice:= MyChoice'(kind => c1_PRESENT, c1 => TRUE);
mySeq : CONSTANT TASTE_BasicTypes.SeqNonOpt:= SeqNonOpt'(
    a => TRUE
    );
NamedInt_one : CONSTANT TASTE_BasicTypes.NamedInt:= 1;
NamedInt_two : CONSTANT TASTE_BasicTypes.NamedInt:= 2;
NamedInt_with_dash_first_value : CONSTANT TASTE_BasicTypes.NamedInt_with_dash:= 0;
NamedInt_with_dash_second_value : CONSTANT TASTE_BasicTypes.NamedInt_with_dash:= 3; 
