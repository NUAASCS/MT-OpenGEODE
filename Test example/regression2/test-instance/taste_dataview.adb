...
BEGIN
    ret := adaasn1rtl.ASN1_RESULT'(Success => ((-100 <= val) AND (val <= 100)), ErrorCode => ERR_SOME_THING);
    RETURN ret;
...
BEGIN
    ...
    while ret.Success and i1 <= val.Length loop
        ...
        ret := adaasn1rtl.ASN1_RESULT'(Success => ((-100 <= val.Data(i1)) AND (val.Data(i1) <= 100)), ErrorCode => ERR_SEQOF_ELM);
        i1 := i1+1;
    end loop;
