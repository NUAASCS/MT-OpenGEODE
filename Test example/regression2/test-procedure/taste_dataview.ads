...
SUBTYPE Enumr_index_range is integer range 0..0;
TYPE Enumr IS (enum1) with Convention => C;
for Enumr use
    (enum1 => 0);

function Enumr_Equal(val1, val2: in Enumr) return Boolean;

function Enumr_Init return Enumr;

ERR_ENUMR:CONSTANT INTEGER := 149; -- 
FUNCTION Enumr_IsConstraintValid(val : in Enumr) return adaasn1rtl.ASN1_RESULT;
