...
package body og is
   ...
   type ctxt_Ty is
      ...
      em : aliased EmptySeq := EmptySeq_Init;
      eso : aliased SeqOf := SeqOf_Init;
      ch : aliased ChoiceWithEmptySeq;
   end record;
      ...
      begin
         while (trId /= -1) loop
            case trId is
               when 0 =>
                  --  ch := b: {} (33,21)
                  ctxt.ch := (Kind => b_PRESENT, b => EmptySeq_Init);
                  --  eso := {} (35,21)
                  ctxt.eso := SeqOf_Init;
                  ...
                  --  ch:= d: {a  TRUE} (42,21)
                  ctxt.ch := (Kind => d_PRESENT, d => ChoiceWithEmptySeq_d'( a => true));
