...
package body og is
   ...
   type ctxt_Ty is
      ...
      seq : aliased SeqOf;
      alwaysfalse : aliased Type2 := false;
      msg : aliased My_OctStr;
      seqofbool : aliased SeqBool := (Data => (1 => true, 2 => true, others => true), Length => 2);
      first_msg : aliased My_OctStr;
      test : aliased Some_Thing;
      foo : aliased MyInteger := 3;
      nonposint : aliased NegativeInt := (-5);
      alwaystrue : aliased Type2 := true;
   end record;
   ...
   procedure runTransition(Id: Integer) is
      ...
      tmp111 : aliased My_OctStr;
      tmp114 : aliased My_OctStr;
      tmp103 : aliased My_OctStr;
      tmp160 : aliased My_OctStr;
      begin
         while (trId /= -1) loop
            case trId is
               when 0 =>
                  ...
                  ctxt.seq.Data(1..ctxt.seq.Length + 2 + default_seqof.Length) := ((((ctxt.seq.Data(1..ctxt.seq.Length)) & SeqOf_Array'(1 => 4, 2 => ctxt.test, others => <>)(1 .. 2))) & default_seqof.Data(1..default_seqof.Length));
                  ...
                  begin
                     for x_idx in 1 .. ctxt.seq.Length loop
                        ...
                        --  writeln(x) (61,9)
                        Put(MyInteger'Image(x));
                        New_Line;
                     end loop;
                  end;
                  --  writeln('test=', test) (64,13)
                  Put("test=");
                  Put(Some_Thing'Image(ctxt.test));
                  New_Line;
               ...
               when 1 =>
                  ...
                  tmp103 := ctxt.msg;
                  --  ANSWER 'egg' (108,17)
                  if My_OctStr_Equal(tmp103, (Data => (101, 103, 103, others => 0), Length => 3)) then
                     --  COMMENT Concatenate
                     --  strings (112,20)
                     --  msg := msg // '!!' (110,25)
                     ctxt.msg.Data(1..ctxt.msg.Length + 2) := ((ctxt.msg.Data(1..ctxt.msg.Length)) & My_OctStr_Array'(33, 33, others => <>)(1 .. 2));
                     ...
                     goto next_transition;
                     --  ANSWER 'end' (125,17)
                  elsif My_OctStr_Equal(tmp103, (Data => (101, 110, 100, others => 0), Length => 3)) then
                     --  rezult
                     --  ('Goodbye!') (127,27)
                     ...
                     --  strings (136,20)
                     --  msg := msg // '!' (134,25)
                     ctxt.msg.Data(1..ctxt.msg.Length + 1) := ((ctxt.msg.Data(1..ctxt.msg.Length)) & My_OctStr_Array'(33, others => <>)(1 .. 1));
               ...
               when 2 =>
                  ...
                  --  ANSWER FALSE (176,17)
                  if (My_OctStr_Equal(ctxt.msg, (Data => (104, 101, 108, 108, 111, others => 0), Length => 5))) = false then
                     ...
                     goto next_transition;
                     --  ANSWER TRUE (184,17)
                  elsif (My_OctStr_Equal(ctxt.msg, (Data => (104, 101, 108, 108, 111, others => 0), Length => 5))) = true then
