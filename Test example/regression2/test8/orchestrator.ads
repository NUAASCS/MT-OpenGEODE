...
package orchestrator is
   ...
   procedure simulate(simulation_choice: access Simulation_Param);
   pragma Export(C, simulate, "orchestrator_PI_simulate");
   --  Provided interface "VESAT_Data"
   procedure VESAT_Data(gnc_inputs: access T_GNC_LV_SIM_CONTEXT);
   pragma Export(C, vesat_data, "orchestrator_PI_vesat_data");
   --  Required interface "VESAT_one_step"
   procedure RIÜVESAT_one_step(vesat_input: access T_GNC_LV_SIM_INPUTS);
   ...
   procedure RIÜgnc_onestep(gnc_inputs: access T_GNC_LV_SIM_CONTEXT; gnc_outputs: access T_GNC_LV_SIM_INPUTS);
