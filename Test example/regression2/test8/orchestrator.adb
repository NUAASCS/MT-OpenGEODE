...
package body orchestrator is
   ...
   type ctxt_Ty is
      ...
      gnc_out : aliased T_GNC_LV_SIM_INPUTS;
      foo : aliased T_QUAT_FLOAT32 := (Data => (1 => 1.0, 2 => 2.0, 3 => 3.0, 4 => 4.0, others => 1.0));
      toto : aliased T_UInt8 := 2;
   end record;
   ...
   procedure simulate(simulation_choice: access Simulation_Param) is
      begin
         case ctxt.state is
            when stopped =>
               runTransition(CS_Only);
   ...
   procedure VESAT_Data(gnc_inputs: access T_GNC_LV_SIM_CONTEXT) is
      begin
         case ctxt.state is
            when stopped =>
               runTransition(CS_Only);
               ...
               when 0 =>
                  ...
                  begin
                     while x < 3 loop
                        ...
                        Put("Hello ");
                        Put(T_FLOAT32'Image(ctxt.foo.Data(1 + Integer(x))));
                        New_Line;
                        --  writeln(x, foo(1)) (20,9)
                        Put(Integer'Image(x));
                        Put(T_FLOAT32'Image(ctxt.foo.Data(2)));
                        New_Line;
                        x := x + 2;
                     end loop;
                  end;
