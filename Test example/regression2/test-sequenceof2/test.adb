         ...
         while (trId /= -1) loop
            case trId is
               when 0 =>
                  ...
                  New_Line;
                  --  s1 := s1 // { false } (37,21)
                  ctxt.s1.Data(1..ctxt.s1.Length + 1) := ((ctxt.s1.Data(1..ctxt.s1.Length)) & MySeqOf_Array'(1 => false, others => <>)(1 .. 1));
                  ctxt.s1.Length := ctxt.s1.Length + 1;
                  --  writeln ( length (s1) ) (39,21)
                  Put(Integer'Image(ctxt.s1.Length));
                  New_Line;
                  --  s1 := s1 // {boo} (41,21)
                  ctxt.s1.Data(1..ctxt.s1.Length + 1) := ((ctxt.s1.Data(1..ctxt.s1.Length)) & MySeqOf_Array'(1 => ctxt.boo, others => <>)(1 .. 1));
                  ctxt.s1.Length := ctxt.s1.Length + 1;
